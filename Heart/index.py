...
class Heart:
    def __init__(self, generate_frame=20):
        self.__points = set()
        self.__edge_diffusion_points = set()
        self.__center_diffusion_points = set()
        self.all_points = {}
        self.build(2000)
    ...
    def build(self, number):
        for _ in range(number):
            ...
            self.__points.add((x, y))

        for _x, _y in list(self.__points):
            for _ in range(3):
                x, y = scatter_inside(_x, _y, 0.3)
                self.__edge_diffusion_points.add((x, y))

        point_list = list(self.__points)
        for _ in range(4000):
            x, y = random.choice(point_list)
            x, y = scatter_inside(x, y, 0.2) 
            self.__center_diffusion_points.add((x, y))
    ...
    def calc(self, generate_frame):
        ...
        halo_radius = int(4 + 6 * (curve(generate_frame / 15 * pi)))
        ...
        for x, y in self.__edge_diffusion_points:
            x, y = self.calc_position(x, y, ratio)
            size = random.randint(1, 2)
            all_points.append((x, y, size))

        for x, y in self.__center_diffusion_points:
            x, y = self.calc_position(x, y, ratio)
            size = random.randint(1, 2)
            all_points.append((x, y, size))
